...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::unprepared("
            ...
            BEGIN
                ...
                DECLARE v_event_day_name VARCHAR(10);
                ...
                SET v_event_day_name = CASE DAYOFWEEK(p_event_date)
                    WHEN 1 THEN 'Minggu'
                    WHEN 2 THEN 'Senin'
                    WHEN 3 THEN 'Selasa'
                    WHEN 4 THEN 'Rabu'
                ...
                read_loop: LOOP
                    FETCH cur_personnel INTO v_personnel_id, v_personnel_name,
                          ...
                          v_day_job_start, v_day_job_end, v_day_job_days, v_is_backup;
                    ...
                    --    Dan mengecek array JSON day_job_days (jika ada, cocokkan hari).
                    IF v_has_day_job = TRUE
                       AND v_day_job_start IS NOT NULL
                       AND v_day_job_end IS NOT NULL
                       AND p_start_time < v_day_job_end
                       AND p_end_time > v_day_job_start 
                       AND (
                           v_day_job_days IS NULL OR 
                           JSON_LENGTH(v_day_job_days) = 0 OR 
                           JSON_CONTAINS(v_day_job_days, CONCAT('\"', v_event_day_name, '\"'))
                       ) THEN
    ...
    {
        // For simplicity, we just leave it as is or revert to previous
        // We'll just leave it empty for down.
    }
};
