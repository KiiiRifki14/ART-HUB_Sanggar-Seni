...
@section('title', 'Edit Katalog Jasa – ART-HUB')
@section('page_title', 'Edit Katalog Jasa')
@section('page_subtitle', 'Perbarui informasi paket jasa: {{ $catalog->name }}.')
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="price" value="{{ old('price', $catalog->price) }}" min="0" required
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="2" required
                                  class="w-full bg-surface-container-low border border-outline-variant/50 rounded-xl px-4 py-2.5 font-body text-sm text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors resize-none">{{ old('description', $catalog->description) }}</textarea>
                    </div>
                    <div>
                        ...
                        <textarea name="detail" rows="4"
                                  class="w-full bg-surface-container-low border border-outline-variant/50 rounded-xl px-4 py-2.5 font-body text-sm text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors resize-none">{{ old('detail', $catalog->detail) }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6">
                    <div class="grid md:grid-cols-2 gap-4 items-start">
                        <div class="rounded-xl overflow-hidden border border-outline-variant/30 bg-surface-container aspect-video">
                            @if($catalog->image)
                                <img src="{{ asset('storage/' . $catalog->image) }}"
                                     class="w-full h-full object-cover" id="previewImg" alt="{{ $catalog->name }}">
                            @else
                        ...
                        <label for="imageInput"
                            ...
                            <div>
                                <p class="font-body text-sm font-bold text-on-surface">Ganti gambar</p>
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <select name="specialty_type" required
                                class="w-full bg-surface-container-low border border-outline-variant/50 rounded-xl px-4 py-2.5 font-body text-sm text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors">
                            <option value="penari"   {{ old('specialty_type', $catalog->specialty_type) === 'penari'   ? 'selected' : '' }}>🎭 Penari saja</option>
                            <option value="pemusik"  {{ old('specialty_type', $catalog->specialty_type) === 'pemusik'  ? 'selected' : '' }}>🎵 Pemusik saja</option>
                            <option value="gabungan" {{ old('specialty_type', $catalog->specialty_type) === 'gabungan' ? 'selected' : '' }}>🎭🎵 Gabungan (Penari + Pemusik)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="max_personnel" value="{{ old('max_personnel', $catalog->max_personnel) }}" min="0"
                               class="w-full bg-surface-container-low border border-outline-variant/50 rounded-xl px-4 py-2.5 font-body text-sm text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors"
                               placeholder="cth: 4">
                    ...
                    <div>
                        ...
                        <input type="text" name="badge" value="{{ old('badge', $catalog->badge) }}" maxlength="50"
                               class="w-full bg-surface-container-low border border-outline-variant/50 rounded-xl px-4 py-2.5 font-body text-sm text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors"
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $catalog->sort_order) }}" min="0"
                               class="w-full bg-surface-container-low border border-outline-variant/50 rounded-xl px-4 py-2.5 font-body text-sm text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors">
