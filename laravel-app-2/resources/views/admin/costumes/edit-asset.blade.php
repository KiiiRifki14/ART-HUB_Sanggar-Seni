...
<div class="max-w-2xl mx-auto">
    <div class="bg-surface-container-lowest rounded-xl border border-outline-variant/30 shadow-sm p-5 md:p-6">
        ...
        <div class="flex items-center gap-3 mb-5 pb-3 border-b border-outline-variant/20">
            ...
            <div>
                <h2 class="font-headline text-base text-primary font-bold">Edit Informasi Aset</h2>
                <p class="font-body text-[0.7rem] text-on-surface-variant">Sesuaikan jumlah, kategori, atau kondisi saat ini dari aset sanggar.</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.costumes.update-asset', $costume->id) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div>
                    ...
                    <select name="category" class="w-full bg-surface-container border border-outline-variant/50 rounded-lg px-3 py-2 font-body text-xs focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors appearance-none" required>
                        ...
                        @php
                            $currentCat = strtolower(old('category', $costume->category));
                        @endphp
                        <option value="atasan" {{ $currentCat == 'atasan' ? 'selected' : '' }}>Atasan</option>
                        <option value="bawahan" {{ $currentCat == 'bawahan' ? 'selected' : '' }}>Bawahan</option>
                        <option value="aksesoris" {{ $currentCat == 'aksesoris' ? 'selected' : '' }}>Aksesoris</option>
                        <option value="alat_musik" {{ $currentCat == 'alat_musik' ? 'selected' : '' }}>Alat Musik</option>
                        <option value="properti" {{ $currentCat == 'properti' ? 'selected' : '' }}>Properti</option>
                        <option value="tradisional" {{ $currentCat == 'tradisional' ? 'selected' : '' }}>Tradisional</option>
                        <option value="modern" {{ $currentCat == 'modern' ? 'selected' : '' }}>Modern</option>
                    </select>
                </div>
                ...
                {{-- Kondisi --}}
                <div class="md:col-span-2">
                    <label class="font-label text-xs font-bold text-on-surface block mb-1 ml-1">Kondisi Saat Ini</label>
                    <div class="grid grid-cols-3 gap-2">
                        <label class="relative flex flex-col p-3 rounded-lg border border-outline-variant/50 cursor-pointer hover:bg-surface-container transition-colors has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                            <input type="radio" name="condition" value="good" class="absolute top-2 right-2 w-3.5 h-3.5 text-primary" {{ old('condition', $costume->condition) == 'good' ? 'checked' : '' }}>
                        ...
                        <label class="relative flex flex-col p-3 rounded-lg border border-outline-variant/50 cursor-pointer hover:bg-surface-container transition-colors has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                            <input type="radio" name="condition" value="damaged" class="absolute top-2 right-2 w-3.5 h-3.5 text-primary" {{ old('condition', $costume->condition) == 'damaged' ? 'checked' : '' }}>
                        ...
                        <label class="relative flex flex-col p-3 rounded-lg border border-outline-variant/50 cursor-pointer hover:bg-surface-container transition-colors has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                            <input type="radio" name="condition" value="maintenance" class="absolute top-2 right-2 w-3.5 h-3.5 text-primary" {{ old('condition', $costume->condition) == 'maintenance' ? 'checked' : '' }}>
            ...
            {{-- Action Buttons --}}
            <div class="pt-4.5 border-t border-outline-variant/20 flex items-center justify-between gap-3">
                <div>
                    {{-- Form Hapus (Delete) dengan konfirmasi --}}
                    <button type="button" onclick="confirmDelete()" class="px-4 py-2 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 font-label text-[0.65rem] font-bold uppercase tracking-widest transition-colors flex items-center gap-1.5">
