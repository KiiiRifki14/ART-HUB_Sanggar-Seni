...
@section('title', 'Nambah Sewaan Vendor – ART-HUB')
@section('page_title', 'Catat Sewaan Vendor')
...
<div class="max-w-2xl mx-auto" x-data="vendorManager">
    <div class="bg-surface-container-lowest rounded-xl border border-outline-variant/30 shadow-sm p-5 md:p-6">
        ...
        <div class="flex items-center gap-3 mb-5 pb-3 border-b border-outline-variant/20">
            ...
            <div>
                <h2 class="font-headline text-base text-secondary font-bold">Detail Penyewaan</h2>
                <p class="font-body text-[0.7rem] text-on-surface-variant">Data ini akan digunakan untuk monitoring pengembalian dan denda.</p>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.costumes.store-rental') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="event_id" class="w-full bg-surface-container border border-outline-variant/50 rounded-lg px-3 py-2 font-body text-xs focus:outline-none focus:border-secondary focus:ring-1 focus:ring-secondary transition-colors appearance-none" required>
                        <option value="" disabled selected>Pilih Event yang sedang aktif</option>
                        @foreach($events as $event)
                            <option value="{{ $event->id }}">{{ $event->booking->client_name }} - {{ $event->booking->event_name }} ({{ \Carbon\Carbon::parse($event->event_date)->format('d M Y') }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="costume_type" placeholder="Misal: Kostum Gatotkaca"
                           class="w-full bg-surface-container border border-outline-variant/50 rounded-lg px-3 py-2 font-body text-xs focus:outline-none focus:border-secondary focus:ring-1 focus:ring-secondary transition-colors" required>
                </div>
            ...
            <div class="pt-4.5 border-t border-outline-variant/20 flex items-center justify-end gap-2.5">
                ...
                <button type="submit" class="px-5 py-2.5 rounded-lg bg-secondary text-white font-label text-[0.65rem] font-bold uppercase tracking-widest hover:bg-secondary-container transition-colors shadow-md flex items-center gap-1.5">
                    <i class="bi bi-clipboard-check"></i> Catat Sewaan
                </button>
            </div>
        </form>
    </div>
